...
void PassRegistry::registerPasses() {
  ...
  //                "async/await style transform, allowing pausing and
  //                resuming", createAsyncifyPass);
  ...
  //     nodes", createDebugLocationPropagationPass);
  //   registerPass("denan",
  ...
  //                "emulates function pointer casts, allowing incorrect
  //                indirect " "calls to (sometimes) work",
  ...
  //     createGlobalStructInferencePass);
  //   registerPass(
  //     "gto", "globally optimize GC types", createGlobalTypeOptimizationPass);
  //   registerPass("gufa",
  //                "Grand Unified Flow Analysis: optimize the entire program
  //                using " "information about what content can actually appear
  //                in each " "location", createGUFAPass);
  ...
  //   registerPass(
  //     "optimize-j2cl", "optimizes J2CL specific constructs.",
  //     createJ2CLOptsPass);
  //   registerPass(
  //     "merge-j2cl-itables",
  ...
  //                "apply more specific subtypes to type fields where
  //                possible", createTypeRefiningPass);
  registerPass(
  ...
  //   registerPass("jspi",
  //                "wrap imports and exports for JavaScript promise
  //                integration", createJSPIPass);
  //   registerPass("legalize-js-interface",
  //                "legalizes i64 types on the import/export boundary",
  //                createLegalizeJSInterfacePass);
  //   registerPass("legalize-and-prune-js-interface",
  //                "legalizes the import/export boundary and prunes when
  //                needed", createLegalizeAndPruneJSInterfacePass);
  ...
  //                " "32-bit one", createMemory64LoweringPass);
  ...
  //                --metrics[=TITLE])", createMetricsPass);
  //   registerPass("minify-imports",
  //                "minifies import names (only those, and not export names),
  //                and " "emits a mapping to the minified ones",
  //                createMinifyImportsPass);
  //   registerPass("minify-imports-and-exports",
  //                "minifies both import and export names, and emits a mapping
  //                to " "the minified ones",
  //                createMinifyImportsAndExportsPass);
  //   registerPass("minify-imports-and-exports-and-modules",
  //                "minifies both import and export names, and emits a mapping
  //                to " "the minified ones, and minifies the modules as well",
  ...
  //                "creates specialized versions of functions (even if
  //                unhelpful)", createMonomorphizeAlwaysPass);
  ...
  //     "combines multiple memories into a single memory, trapping if the read
  //     or " "write is larger than the length of the memory's data",
  //     createMultiMemoryLoweringWithBoundsChecksPass);
  //   registerPass("nm", "name list", createNameListPass);
  //   registerPass("name-types", "(re)name all heap types",
  //   createNameTypesPass); registerPass("no-inline", "mark functions as
  //   no-inline", createNoInlinePass); registerPass("no-full-inline",
  ...
  //                "optimizes added constants into load/store offsets,
  //                propagating " "them across locals too",
  ...
  // // Outlining currently relies on LLVM's SuffixTree, which we can't rely
  // upon
  ...
  //   // Also register it as "symbolmap" so that  wasm-opt --symbolmap=foo  is
  //   the
  ...
  //                "propagate global values to other globals (useful for
  //                tests)", createPropagateGlobalsGloballyPass);
  ...
  //                    "sorts globals by access frequency (even if there are
  //                    few)", createReorderGlobalsAlwaysPass);
  ...
  //                "write data segments to a file and strip them from the
  //                module", createSeparateDataSegmentsPass);
  //   registerPass("signature-pruning",
  //                "remove params from function signature types where
  //                possible", createSignaturePruningPass);
  //   registerPass("signature-refining",
  //                "apply more specific subtypes to signature types where
  //                possible", createSignatureRefiningPass);
  //   registerPass("signext-lowering",
  //                "lower sign-ext operations to wasm mvp and disable the sign
  //                " "extension feature", createSignExtLoweringPass);
  ...
  //                "miscellaneous locals-related optimizations (no nesting at
  ...
  //   registerPass("souperify", "emit Souper IR in text form",
  //   createSouperifyPass); registerPass("souperify-single-use",
  ...
  //     imports", createStringLoweringMagicImportPass);
  //   registerPass("string-lowering-magic-imports-assert",
  //                "same as string-lowering-magic-imports, but raise a fatal
  //                error " "if there are invalid strings",
  ...
  //   registerPass("strip-dwarf", "strip dwarf debug info",
  //   createStripDWARFPass); registerPass("strip-producers",
  ...
  //                "translate old Phase 3 EH instructions to new ones with
  //                exnref", createTranslateToExnrefPass);
  ...
  //   // registerPass(
  //   //   "lower-i64", "lowers i64 into pairs of i32s",
  //   createLowerInt64Pass);

  //   // Register passes used for internal testing. These don't show up in
  //   --help. registerTestPass("catch-pop-fixup",
